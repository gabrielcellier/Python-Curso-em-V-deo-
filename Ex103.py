...
def ficha(jogador='desconhecido', gols=0):
    ...
    if gols == '':
        gols == '0'
    return f'O jogador {jogador} fez {gols} gols na competição.'

j = str(input('Nome do jogador: ')).capitalize().strip()
g = str(input(f'Gols que o {j} marcou (apenas números): ')).strip()   #usamos str para poder aceitar valor caso usuário escreva por extenso
